...
- The channel button and the volume slider follow it too. On the marker voice the button picks which of the machine's text to speech voices does the talking, each pick saying a word so it can be judged right there, and the slider becomes the text to speech volume. The follower voice for the leader's calls speaks with the same pick and volume
- Turning the spoken cues off now greys everything under it, the voice pick, the test buttons, the channel and the volume, instead of leaving live controls for a voice that will not speak
- The arrow's compass toggle became a sliding two state switch, Relative on one side and Compass on the other. The pill and knob are svg art, which the 12.1 client renders straight from disk, with a plain drawn stand-in on clients that cannot

## 1.6.0
- Answer in relative turns, a new way to record for anyone who thinks in moves rather than map directions. With the toggle on, your quarter keys say the move you made instead of the place you ended up, read the same way the arrow and the voice already speak: north is straight through the boss, east and west are your right and left as you face him. The first wave still names its quarter, since there is nothing behind it to turn from, and that is where the route starts. South sits idle after that, because the safe spot never lands in the same quarter twice in a row. Marking and calling need a named quarter, so they switch off while the toggle is on. `/azt turns` flips it from chat, room view clicks keep naming quarters in both modes
- The Preview button grew into a full practice drill. It now runs a pretend sermon on the room view, seven waves with the safe quarter green and the rest red, and you answer each one with your keys or clicks exactly like a real pull, relative turns included. A short countdown opens it, saying along the way whether your keys are answering turns or compass quarters, so your hands are on the right keys before wave one. The echoes that follow play back whatever you recorded, it tells you how many of the seven you got right, and `/azt practice` starts the same drill from chat. The route is random every time, so it stays a memory drill rather than a memorized one
- Two guards against slipped keys, in both modes. A second press of the same key within a second or so is dropped as one fumbled press, since real waves never come that fast, and an answer that would repeat the quarter next to it is refused outright, because the boss never calls the same quarter twice in a row
- Speak the leader's calls now greys out unless following is on, since the voice only ever has the leader's calls to read
