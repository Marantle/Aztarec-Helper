...
- Follow the leader is the other side: the leader's calls show as marker icons on a route board and on the wave countdown, timed off the boss like always. In the fight chat is sealed to addons, so the calls can be shown but never read. That is why the arrow and the solo cues lock off while following, with your own settings returning untouched when you stop, and why party chat during the fight should stay quiet, since stray lines land on the board as garbage
...
- Calls come in two languages, picked by whoever leads. Markers name the quarter's icon by number, or switch to directions and the calls read Up, Right, Down and Left with the room north up like the room view draws it. Followers draw directions as arrows instead of marker icons, and the words mean something to party members without the addon as well. The choice travels with the leader's icon sync, so every board knows what it is drawing before the pull
- The direction arrows are a quarter language of their own, picked from the same menu as the markers on the room view or from the settings. Take it and every quarter reads as its arrow, on the room view, the countdown and the arrow itself, and your calls say the words to match. Marking yourself still uses the quarter's marker, since a target icon is the only thing the game will put on a player
...
- The settings panel grew into sections and a second column, and it scrolls now so a long column cannot run off the bottom of the canvas. The windows, the sound and the route buttons sit on the left while the quarter keys, the party options and the window locks sit on the right
