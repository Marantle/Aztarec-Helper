# Azta'rec Helper

Helper for the Azta'rec delve nemesis in Venomfall Deeps (Midnight 12.1). The
boss runs a memory game: waves sweep the room with visible markers, then repeat
without them. The addon records which quarter was safe during each marked wave
and guides you through the blind repeat. The full feature rundown lives on the
[CurseForge page](https://www.curseforge.com/wow/addons/aztarec-helper).

Everything shows up on its own when it is needed. `/azt` lists the commands and
`/azt options` opens the settings. Manual capture has a bindable key under
AddOns in the key bindings panel.

you can search this repo for `--#region` to find different logic regions in the code
like `--#region Locks` defines an area start for the logic to locking the panels in place